---
title: Terms and Conditions
---

# Terms and Conditions

**BrainDock Focus Tracking Application**

Last Updated: January 2026

---

## 1. Agreement to Terms

By downloading, installing, accessing, or using BrainDock ("the Application", "the App", "we", "us", or "our"), you ("User", "you", or "your") agree to be bound by these Terms and Conditions ("Terms"). If you do not agree to these Terms, do not use the Application.

These Terms constitute a legally binding agreement between you and BrainDock. Please read them carefully.

---

## 2. Description of Service

BrainDock is a focus tracking application designed to help users monitor their study or work sessions. The Application:

- Uses your device's camera to detect your presence and potential distractions
- Monitors your active window and browser URLs to detect distraction apps/websites
- Optionally captures screenshots for AI analysis (disabled by default, user-controlled)
- Sends camera frames and/or screenshots to third-party AI services for real-time analysis
- Generates session reports summarising your focus patterns
- Does **NOT** perform facial recognition, identity verification, or biometric identification

### 2.1 What BrainDock Does

- Detects whether a person is present at the desk
- Detects whether you appear to be using distraction devices (phones, tablets, game controllers, etc.)
- Monitors active window titles and browser URLs to detect distraction apps/websites
- **Optional AI Screenshot Analysis**: If enabled by you, captures screenshots and sends them to AI for distraction detection (disabled by default)
- Generates focus session statistics and PDF reports
- Provides AI-generated insights about your focus patterns

### 2.2 What BrainDock Does NOT Do

- **No Facial Recognition**: We do not identify who you are
- **No Biometric Templates**: We do not create, store, or match biometric data
- **No Identity Verification**: We do not verify or match your identity across sessions
- **No Video Recording**: We do not save or store video footage
- **No Screenshot Storage**: Screenshots (if enabled) are processed in real-time and not saved
- **No Continuous Surveillance**: Processing only occurs during active sessions you initiate
- **No Keystroke Logging**: We do not record what you type

---

## 3. Eligibility

### 3.1 Age Requirements

You must be at least 13 years of age to use this Application. If you are under 18, you must have permission from a parent or legal guardian who agrees to these Terms on your behalf.

### 3.2 Capacity

By using the Application, you represent that you have the legal capacity to enter into this agreement.

---

## 4. User Responsibilities

### 4.1 Your Device and Environment

You are responsible for:

- Ensuring you have the legal right to use camera capture on your device
- The content captured by your camera during sessions
- Obtaining consent from any third parties who may be captured by your camera

### 4.2 Third-Party Consent Warning

**IMPORTANT**: The Application captures camera frames from your device. If other people (housemates, family members, colleagues, clients on video calls, or passersby) may appear in your camera's view:

- **You are responsible for obtaining their consent** before starting a session
- Consider positioning your camera to minimise capturing others
- Use the Application in a private space where practical
- Do not use the Application to monitor others without their knowledge and consent

We are not responsible for any privacy violations arising from your capture of third parties.

### 4.3 Prohibited Uses

You agree NOT to use the Application:

- To covertly monitor or surveil other individuals
- In violation of any applicable laws or regulations
- To capture or process images of minors without parental consent
- In any manner that infringes the rights of others
- To circumvent, disable, or interfere with security features

### 4.4 Workplace Use

If you use BrainDock in a workplace context (whether as an employee or employer):

- **Employees**: Ensure your employer permits personal software on work devices
- **Employers deploying to staff**: You must comply with workplace surveillance laws in your jurisdiction

**Australian Employers - Special Notice**: If deploying BrainDock to monitor employees in Australia, you must comply with state and territory workplace surveillance legislation, including but not limited to:

- **NSW**: Workplace Surveillance Act 2005 - requires written notice at least 14 days before surveillance begins, specifying the kind of surveillance, how it will be carried out, when it starts, and whether it is continuous or intermittent
- **VIC**: Surveillance Devices Act 1999
- **QLD**: Invasion of Privacy Act 1971
- **Other states**: Applicable surveillance device legislation

**We strongly recommend employers seek independent legal advice before deploying BrainDock for employee monitoring.**

---

## 5. Licence and Access

### 5.1 Licence Grant

Subject to these Terms and payment of applicable fees, we grant you a limited, non-exclusive, non-transferable, revocable licence to use the Application for personal, non-commercial purposes.

### 5.2 Licence Restrictions

You may not:

- Copy, modify, or distribute the Application
- Reverse engineer, decompile, or disassemble the Application
- Rent, lease, lend, sell, or sublicense the Application
- Use the Application to develop competing products
- Remove or alter any proprietary notices

### 5.3 Licence Termination

We may terminate your licence if you breach these Terms. Upon termination, you must cease all use of the Application and delete all copies.

---

## 6. Payment and Refunds

### 6.1 Pricing

The Application requires a one-time payment for full access. Current pricing is displayed at the point of purchase.

### 6.2 Payment Processing

Payments are processed securely through Stripe. By making a payment, you also agree to Stripe's terms of service.

### 6.3 Refund Policy

Given the nature of digital software, refunds are provided at our discretion. You may request a refund within 7 days of purchase if:

- The Application fails to function as described
- You experience technical issues we cannot resolve
- You have not generated any session reports

To request a refund, contact us with your purchase details.

---

## 7. Data Processing Acknowledgment

By using the Application, you acknowledge and consent to:

### 7.1 Camera Frame Processing

- Camera frames are captured during active sessions
- Frames are transmitted to third-party AI services for analysis. We use one of the following providers at a time:
  - **OpenAI** (servers located in the United States)
  - **Google Gemini** (servers located in the United States)
- These providers may retain data for up to 30 days for safety and abuse monitoring per their respective policies
- We do not store raw camera frames on your device or our servers

### 7.2 Screen Monitoring

- The Application monitors your active window titles and browser URLs to detect distraction apps/websites
- This data is processed locally on your device and compared against your blocklist
- Window titles and URLs are NOT sent to third-party services (unless AI Screenshot Analysis is enabled)

### 7.3 Optional AI Screenshot Analysis

- This feature is **disabled by default** and requires you to explicitly enable it
- When enabled, the Application captures screenshots of your screen
- Screenshots are transmitted to our AI providers (OpenAI or Google Gemini) for distraction analysis
- Screenshots are processed in real-time and **NOT stored** on your device or our servers
- You can disable this feature at any time in the Application settings
- **Warning**: Screenshots may capture sensitive information visible on your screen

### 7.4 Session Data

- Session logs (timestamps, event types, focus statistics) are stored locally on your device
- PDF reports are generated and saved to your device
- You control the retention and deletion of local data

### 7.5 Cross-Border Data Transfer

Your camera frames and screenshots (if enabled) are transmitted to AI service providers located outside Australia. Our current providers (OpenAI and Google) operate servers in the United States. By using the Application, you consent to this cross-border transfer. See our Privacy Policy for details on data protection measures.

For full details on data handling, please review our [Privacy Policy](PRIVACY_POLICY.md).

---

## 8. No Biometrics Pledge

We explicitly confirm that BrainDock:

- Does **NOT** perform facial recognition or identification
- Does **NOT** create biometric templates or profiles
- Does **NOT** match or verify user identity
- Does **NOT** use biometric information for automated verification or identification
- Processes only general presence detection (is someone at the desk?) and object detection (is a phone visible?)

This pledge is a fundamental design principle of our Application.

---

## 9. Intellectual Property

### 9.1 Our Property

The Application, including its code, design, features, documentation, and branding, is owned by BrainDock and protected by intellectual property laws.

### 9.2 Your Content

You retain ownership of any content you create using the Application (such as session reports). By using the Application, you grant us no rights to your generated content.

---

## 10. Third-Party Services

### 10.1 AI Processing Services

The Application uses third-party AI services for image analysis. We currently use one of the following providers at any given time:

**OpenAI:**
- OpenAI Terms of Use: https://openai.com/terms
- OpenAI Privacy Policy: https://openai.com/privacy
- OpenAI API Data Usage Policy: https://openai.com/policies/api-data-usage-policies

**Google (Gemini):**
- Google Terms of Service: https://policies.google.com/terms
- Google Privacy Policy: https://policies.google.com/privacy
- Google Cloud Data Processing Terms: https://cloud.google.com/terms/data-processing-terms

### 10.2 Payment Processing

Payments are processed by Stripe:

- Stripe Terms of Service: https://stripe.com/legal
- Stripe Privacy Policy: https://stripe.com/privacy

---

## 11. Disclaimers

### 11.1 "As Is" Provision

THE APPLICATION IS PROVIDED "AS IS" AND "AS AVAILABLE" WITHOUT WARRANTIES OF ANY KIND, EXPRESS OR IMPLIED. WE DISCLAIM ALL WARRANTIES INCLUDING, BUT NOT LIMITED TO, IMPLIED WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, AND NON-INFRINGEMENT.

### 11.2 No Guarantee of Results

We do not guarantee that:

- The Application will improve your focus or productivity
- Detection will be 100% accurate
- The Application will be uninterrupted or error-free
- The Application will meet your specific requirements

### 11.3 Not Medical or Therapeutic Advice

BrainDock is a productivity tool, not a medical device or therapeutic intervention. The Application:

- Does not provide medical, psychological, or therapeutic advice
- Is not intended to diagnose, treat, or manage any medical condition including ADHD, anxiety, or other conditions
- Should not be used as a substitute for professional medical advice

If you have concerns about focus, attention, or mental health, please consult a qualified healthcare professional.

---

## 12. Limitation of Liability

### 12.1 Exclusion of Damages

TO THE MAXIMUM EXTENT PERMITTED BY LAW, IN NO EVENT SHALL BRAINDOCK BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL, CONSEQUENTIAL, OR PUNITIVE DAMAGES, INCLUDING BUT NOT LIMITED TO:

- Loss of profits, data, or goodwill
- Business interruption
- Personal injury or emotional distress
- Privacy breaches caused by your use of the Application
- Any damages arising from third-party actions

### 12.2 Cap on Liability

OUR TOTAL LIABILITY FOR ALL CLAIMS ARISING FROM OR RELATING TO THESE TERMS OR THE APPLICATION SHALL NOT EXCEED THE AMOUNT YOU PAID FOR THE APPLICATION IN THE 12 MONTHS PRECEDING THE CLAIM.

### 12.3 Australian Consumer Law

Nothing in these Terms excludes, restricts, or modifies any consumer guarantee, right, or remedy conferred by the Australian Consumer Law (Schedule 2 of the Competition and Consumer Act 2010) that cannot be excluded, restricted, or modified by agreement.

---

## 13. Indemnification

You agree to indemnify and hold harmless BrainDock and its officers, directors, employees, and agents from any claims, damages, losses, or expenses (including legal fees) arising from:

- Your use of the Application
- Your breach of these Terms
- Your violation of any third party's rights, including privacy rights
- Your capture of third parties without consent
- Your use of the Application in violation of workplace surveillance laws

---

## 14. Modifications to Terms

### 14.1 Right to Modify

We reserve the right to modify these Terms at any time. Changes will be effective when posted.

### 14.2 Notification

We will notify you of material changes through:

- In-app notification
- Email (if you have provided one)
- Updated "Last Updated" date on these Terms

### 14.3 Continued Use

Your continued use of the Application after changes constitutes acceptance of the modified Terms. If you do not agree to changes, you must stop using the Application.

---

## 15. Termination

### 15.1 By You

You may stop using the Application at any time by uninstalling it from your device.

### 15.2 By Us

We may terminate or suspend your access immediately, without prior notice, for any reason, including breach of these Terms.

### 15.3 Effect of Termination

Upon termination:

- Your licence to use the Application ends
- You must delete all copies of the Application
- Provisions that by their nature should survive (including disclaimers, limitations of liability, and indemnification) will survive

---

## 16. Governing Law and Disputes

### 16.1 Governing Law

These Terms are governed by and construed in accordance with the laws of Victoria, Australia.

### 16.2 Jurisdiction

You agree to submit to the exclusive jurisdiction of the courts of Victoria, Australia, for any disputes arising from or relating to these Terms or the Application.

### 16.3 Informal Resolution

Before initiating formal proceedings, you agree to contact us to attempt to resolve any dispute informally.

---

## 17. General Provisions

### 17.1 Entire Agreement

These Terms, together with our Privacy Policy, constitute the entire agreement between you and BrainDock regarding the Application.

### 17.2 Severability

If any provision of these Terms is found to be unenforceable, the remaining provisions will continue in effect.

### 17.3 Waiver

Our failure to enforce any right or provision of these Terms will not constitute a waiver of that right or provision.

### 17.4 Assignment

You may not assign or transfer these Terms without our prior written consent. We may assign our rights and obligations without restriction.

---

## 18. Contact Us

If you have questions about these Terms, please contact us at:

**Email**: morayyajain@gmail.com

**Website**: thebraindock.com

**Address**: 108-128 Leicester Street, Carlton, VIC-3053

---

## 19. Acceptance

By clicking "I Agree", completing a purchase, or using the Application, you acknowledge that you have read, understood, and agree to be bound by these Terms and Conditions and our Privacy Policy.

---

*These Terms and Conditions were last updated in January 2026.*
